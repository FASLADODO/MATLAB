...
asizesdif=abs(max(asdif(:))-min(asdif(:)));
amulsdif=(2*exp(1))/asizesdif;
aconsdif=-1*exp(1)-amulsdif*min(asdif(:));
acval=normcdf(amulsdif.*asdif+aconsdif,0,1);

hsizesdif=abs(max(hsdif(:))-min(hsdif(:)));
hmulsdif=(2*exp(1))/hsizesdif;
hconsdif=-1*exp(1)-hmulsdif*min(hsdif(:));
hcval=normcdf(hmulsdif.*sdif+hconsdif,0,1);
...
omout=['./nc_mat/EchoTop18' num2str('%02i'n-1,) ];
hmout=['./nc_mat/HRRR_EchoTop_18' num2str('%02i',n-1) ];
save(omout,'EchoTop_18');
save(hmout,'HRRR_EchoTop_18');
