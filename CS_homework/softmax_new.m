...
% valnA=[-2; -1; 0; 1; 2;];
% qStarMeans = zeros(nB,nA);
% for qi=1:nA
%     for qj=1:nB
%         qStarMeans(qi,qj)=valnA(qj);
%     end
% end
qStarMeans=[-2; -1; 0; 1; 2;];
...
%   for bi=1:nB, % pick a bandit
    for pi=1:nP, % make a play
      ...
      if( arm==bestArm ) pickedMaxAction(pi) = 1; end
      % get the reward from drawing on that arm: 
      reward = qStarMeans(arm) + sigma*randn(1); 
      allRewards(pi) = reward; 
      % update qN,qS,qT: 
      qN(arm) = qN(arm)+1;
      qS(arm) = qS(arm)+reward; 
      qT(arm) = qS(arm)/qN(arm); 
    end
%   end
