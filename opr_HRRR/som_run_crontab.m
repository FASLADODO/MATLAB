...
[fdate2] = textread('listdate2.dat', '%d \n');
    fo = fopen([ './somscript/crontab_2015_APR.txt'],'w');
...
for ii=1:numel(fdate)
    ...
    a2=num2str(fdate2(ii));
           if ij-1+3>=24
        fprintf(fo,'%s\n',[ '30 ' num2str(ij-1+3-24,'%02d') ' ' a2(7:8) ' ' a2(5:6)  ' * bash /webhost/akahwang/http/shell/som' num2str(fdate(ii),'%08d') ...
        '_' num2str(ij-1,'%02d') '.sh > /webhost/akahwang/http/log/' num2str(fdate(ii),'%08d') '_' num2str(ij-1,'%02d') '.log 2>&1' ]);
           else
        fprintf(fo,'%s\n',[ '30 ' num2str(ij-1+3,'%02d') ' ' a1(7:8) ' ' a1(5:6)  ' * bash /webhost/akahwang/http/shell/som' num2str(fdate(ii),'%08d') ...
        '_' num2str(ij-1,'%02d') '.sh > /webhost/akahwang/http/log/' num2str(fdate(ii),'%08d') '_' num2str(ij-1,'%02d') '.log 2>&1' ]);
