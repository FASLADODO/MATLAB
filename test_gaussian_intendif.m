...
mFK=(((nk-(n-1))/nk.*aFK)+((1-(nk-(n-1))/nk).*hFK))./2;
mFK2=zeros(size(mFK));
[nki,nkj]=size(cval);
for ki=1:nki
    for kj=1:nkj
...
caxis([0 1])
shading flat;
title(['t=' num2str(n-1) ',ADV intensity']);
...
caxis([0 1])
shading flat;
title(['t=' num2str(n-1) ',HRR intensity']);
...
caxis([-1 1])
shading flat;
title(['t=' num2str(n-1) ',DIF intensity']);
